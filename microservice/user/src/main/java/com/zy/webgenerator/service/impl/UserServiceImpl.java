...
import com.zy.webgenerator.mapper.UserMapper;
import com.zy.webgenerator.model.dto.user.UserQueryRequest;
import com.zy.webgenerator.model.entity.User;
import com.zy.webgenerator.model.enums.UserRoleEnum;
import com.zy.webgenerator.model.vo.LoginUserVO;
...
import static com.zy.webgenerator.constant.UserConstant.USER_LOGIN_STATE;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>  implements UserService {
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        ...
        // 3. 加密密码
        String encrptPassword = getEncryptPassword(userPassword);
        // 4. 创建用户，加入数据库
        User user = new User();
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword, HttpServletRequest request) {
        ...
        if (userAccount.length() < 4) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "account length is too short");
        }
        if (userPassword.length() < 8) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "password is too short or no such user");
        }
        //2.加密
        String encrptPassword = getEncryptPassword(userPassword);
        //3.查询用户是否存在
        ...
        //用户不存在
        if (user == null){
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "no such user or wrong password");
        }
        //记录用户的登录态
    ...
    public User getLoginUser(HttpServletRequest request) {
        ...
        if (currentUser == null || currentUser.getId() == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN_ERROR, "user not login");
        }
        //从数据库查询当前用户信息
